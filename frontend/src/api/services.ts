...
export async function getSummary(): Promise<SummaryResponse> {
  if (isMockMode) return mock(demoSummary, { totalNodes: 0, totalEdges: 0, totalTransactions: 0, totalSeeds: 0, totalClusters: 0, roles: { coordinator: 0, consolidator: 0, distributor: 0, transit: 0, terminal: 0, peripheral: 0 }, topNodes: [], topClusters: [] })
...
export async function getNode(gid: Gid): Promise<NodeDetails> {
  ...
  await mockDelay()
  if (mockScenario === 'error') throw new Error('Demonstration API error. Retry or select another mock scenario.')
  if (mockScenario === 'empty') throw new Error(`GID ${gid} was not found in the empty demonstration scenario.`)
  const node = demoNodes.find((item) => item.gid === gid)
  if (!node) throw new Error(`GID ${gid} was not found in demonstration data.`)
  return structuredClone(node)
}

export async function getClusters(): Promise<ClusterListResponse> {
  if (isMockMode) return mock({ items: demoClusters, total: demoClusters.length }, { items: [], total: 0 })
...
export async function getCluster(clusterId: number): Promise<ClusterDetails> {
  ...
  if (!cluster) throw new Error(`Cluster ${clusterId} was not found in demonstration data.`)
